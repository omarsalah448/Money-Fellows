    ...
    <?php try {
        if ($_GET['exists'])?>
            <div class="alert alert-warning" role="alert">
                You are already enrolled in this Gameeya !
              </div>
        <?php 
    } catch (\Throwable $th) {
        //throw $th;
                ...
                <div class="col-lg-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <?php
                            $count=DB::table('user_currents')->where('current_id',$item->id)->count();
                            $members=DB::table('categories')->where('id',$item->category_id)->first();
                            $members=$members->no_users;
                            ?>
                            @if($count<$members)
                            <div class="button-wrapper">
                                <form action="{{ route('userCurrent.store', "current_id=$item->id" ) }}" method="post">
                                    @csrf
                                    <button class="btn-solid-reg page-scroll" type="submit">JOIN</button>
                                </form>
                                </div>
                            @else
                            <div class="button-wrapper">
                                <form action="/user" method="get">
                                    <button class="btn-solid-reg page-scroll" type="submit">FULL</button>
                                </form>
                            </div>
                            @endif    
                            @endif
